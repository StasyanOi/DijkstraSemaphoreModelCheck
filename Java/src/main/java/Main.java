...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 5; i++) {
            for (int j = i + 1; j <= 5; j++) {
                //for (int k = j + 1; k <= 5; k++) {
                    procLtl.add(new int[]{i, j});
                //}
            }
        }
//        (proc1.state = critical & proc2.state = critical & proc3.state = critical)
        String collect = procLtl.stream()
                .map(ints -> " F ((proc" + ints[0] + ".state = critical1 | proc " + "proc" + ints[0] + ".state = critical2)" +
                        " & F (proc" + ints[1] + ".state = critical1 | " + "proc" + ints[1] + ".state = critical2)")
