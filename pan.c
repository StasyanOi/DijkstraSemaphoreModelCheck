...
/*** From source: ./code/processes.pml ***/
...
short src_ln4 [] = {
	  0,  28,  28,  29,  29,  29,  30,  30, 
	 27,  32,  27,  34,  34,  33,  36,  33, 
	 38,  38,  38,  39,  39,  37,  41,  37, 
	 42,  43,   0, };
S_F_MAP src_file4 [] = {
	{ "-", 0, 0 },
	{ "_spin_nvr.tmp", 1, 25 },
...
short src_ln0 [] = {
	  0,  62,  16,  17,  18,  15,  14,  39, 
	 40,  40,  41,  41,  39,  43,  39,  37, 
	 47,  45,  24,  25,  23,  22,  50,  64, 
	 65,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "./code/processes.pml", 1, 24 },
	{ "-", 25, 26 }
};
uchar reached0 [] = {
	  0,   0,   1,   0,   0,   1,   1,   1, 
...
{	int j = 0, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 6:	/* claim select */
		spin_c_typ[0] = 4; /* task4 */
		((P6 *)pptr(h))->c_cur[0] = 8;
		reached4[8]=1;
		spin_c_typ[1] = 3; /* task3 */
		((P6 *)pptr(h))->c_cur[1] = 6;
		reached3[6]=1;
		spin_c_typ[2] = 2; /* task2 */
		((P6 *)pptr(h))->c_cur[2] = 3;
		reached2[3]=1;
		spin_c_typ[3] = 1; /* task1 */
		((P6 *)pptr(h))->c_cur[3] = 6;
		reached1[6]=1;
		((P6 *)pptr(h))->_t = 1;
		((P6 *)pptr(h))->_p = 6;
		((P6 *)pptr(h))->_n = 3; /* task1 */
		src_claim = src_ln1;
#ifndef BFS
		if (whichclaim == -1 && claimname == NULL)
			printf("pan: ltl formula task1\n");
#endif
		if (whichclaim != -1)
	...
	case 0:	/* main */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("main:count", ((P0 *)pptr(h))->_1_1_2_2_count);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.semaphoreThreshold = 3;
		now.semaphoreTokens = now.semaphoreThreshold;
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			now.wasInCriticalSection[l_in] = 0;
		}
	}
#ifdef VAR_RANGES
		logval("semaphoreThreshold", now.semaphoreThreshold);
		logval("semaphoreTokens", now.semaphoreTokens);
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			logval("wasInCriticalSection[l_in]", now.wasInCriticalSection[l_in]);
		}
	}
#endif
}
...
{	/* int i; */
	printf("global vars:\n");
	printf("	int    semaphoreThreshold:	%d\n", now.semaphoreThreshold);
	printf("	int    semaphoreTokens:	%d\n", now.semaphoreTokens);
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			printf("	int    wasInCriticalSection[%d]:	%d\n", l_in, now.wasInCriticalSection[l_in]);
		}
	}
}
void
